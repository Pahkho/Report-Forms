...
--   AND (stock_date BETWEEN '${date_begin}' AND '${date_end}' )
  AND (trade_purchase_bill.stock_date BETWEEN '2022-01-01' AND '2022-12-31' )
#   AND ( bas_supplier.name LIKE ('%T%')           -- 客户/供应商名称
#   or bas_supplier.code LIKE ('%T%'))
# --   AND ('1' IN ('${queryType}'))               -- 查询类型
#   AND ('1' IN ('${boundType}')
#            OR '0' IN ('${boundType}'))               -- 出入库类型
--  AND style_color LIKE ('%style_color%')      -- 花号
...
where trade_purchase_return_bill.delivery_status = 8
  ...
  AND trade_purchase_return_bill.stock_date BETWEEN '2022-01-01' AND '2022-03-31'
# --    AND bas_supplier.code LIKE ('')        -- 客户/供应商名称
#   AND ('1' IN ('${queryType}'))            -- 查询类型
...
where trade_sale_bill.delivery_status = 8
  ...
  AND trade_sale_bill.stock_date BETWEEN '2022-01-01' AND '2022-03-31'
# --    AND bas_customer.code LIKE ('')        -- 客户/供应商名称
#   AND ('1' IN ('${queryType}'))            -- 查询类型
...
select ''                                                                                                 AS stock_date,    -- 日期
       ...
       ''                                                                                                 AS coding,        -- 编码
       ''                                                                                                 AS gb_coding,     -- 国标码
       sum(CASE WHEN stk_stklogic_adjbill_detail.qty > 0 THEN stk_stklogic_adjbill_detail.qty ELSE 0 END) AS in_qty,         -- 入库数量
       sum(CASE WHEN stk_stklogic_adjbill_detail.qty < 0 THEN stk_stklogic_adjbill_detail.qty ELSE 0 END) AS out_qty         -- 出库数量
...
where stk_stock_logic_adjust_bill.bill_status = '05'
  ...
  AND stk_stock_logic_adjust_bill.stock_date BETWEEN '2022-01-01' AND '2022-03-31'
# --    AND bas_supplier.code LIKE ('')        -- 客户/供应商名称
#   AND ('1' IN ('${queryType}'))            -- 查询类型
...
where drp_cons_sale_bill.bill_status = '05'
  ...
  AND drp_cons_sale_bill.stock_date BETWEEN '2022-01-01' AND '2022-03-31'
# --    AND bas_supplier.code LIKE ('')        -- 客户/供应商名称
#   AND ('1' IN ('${queryType}'))            -- 查询类型
...
where drp_cons_return_bill.bill_status = '05'
  ...
  AND drp_cons_return_bill.stock_date BETWEEN '2022-01-01' AND '2022-03-31'
# --    AND bas_supplier.code LIKE ('')        -- 客户/供应商名称
#   AND ('1' IN ('${queryType}'))            -- 查询类型
...
where trade_sale_return_bill.delivery_status = '13'
  ...
  AND trade_sale_return_bill.stock_date BETWEEN '2022-01-01' AND '2022-03-31'
# --    AND bas_supplier.code LIKE ('')        -- 客户/供应商名称
#   AND ('1' IN ('${queryType}'))            -- 查询类型
...
where stk_allocate_out_bill.status = '05'
  ...
  AND stk_allocate_out_bill.stock_date BETWEEN '2022-01-01' AND '2022-03-31'
# --    AND bas_supplier.code LIKE ('')        -- 客户/供应商名称
#   AND ('1' IN ('${queryType}'))            -- 查询类型
...
where wms_stock_out_bill.status = '05'
  ...
  AND wms_stock_out_bill.out_warehouse_date BETWEEN '2022-01-01' AND '2022-03-31'
# --    AND bas_supplier.code LIKE ('')        -- 客户/供应商名称
#   AND ('1' IN ('${queryType}'))            -- 查询类型
