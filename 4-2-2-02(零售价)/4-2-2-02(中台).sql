SELECT
	...
	sum(begin_qty) + sum(in_qty) - sum(issue_qty) + sum(profit_loss_qty) - sum(return_qty) AS 期末数量,
	round(sum(begin_value) + sum(in_value) - sum(issue_value) + sum(profit_loss_value) - sum(return_value),2) AS 期末金额
FROM
	(
		SELECT
	c1.CODE goods_code,
	...
	0 end_value
FROM
	fin_cst_pac_item_costs_summary a1
	...
	FROM
		fin_cst_pac_item_costs_summary a
	...
	WHERE
		((DATE_FORMAT(a.period_start_date,'%Y-%m') BETWEEN '@startEndDate_START' AND '@startEndDate_END') OR ('@startEndDate_START'='@' AND '@startEndDate_END'='@'))
	GROUP BY
		a.summary_item_id
	) d -- 获取最新取价
...
WHERE
		((DATE_FORMAT(a1.period_start_date,'%Y-%m') BETWEEN '@startEndDate_START' AND '@startEndDate_END') OR ('@startEndDate_START'='@' AND '@startEndDate_END'='@'))
GROUP BY
	c1.CODE,
	c1.NAME
		UNION ALL
			...
			INNER JOIN (
				...
				WHERE
					a.assigner_id = '10001'
				AND a.qty_type = 11
				AND ((a.create_time BETWEEN '@startEndDate_START' AND '@startEndDate_END') OR ('@startEndDate_START'='@' AND '@startEndDate_END'='@'))
			...
			AND e.modify_time = d.max_time
			WHERE
				a1.assigner_id = '10001'
			AND a1.qty_type = 11
			AND ((a1.create_time BETWEEN '@startEndDate_START' AND '@startEndDate_END') OR ('@startEndDate_START'='@' AND '@startEndDate_END'='@'))
