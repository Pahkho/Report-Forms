...
FROM (
         ...
         FROM stk_stock_logic_workflow sslw
             ...
             SELECT mpc1.*
             FROM (
                      SELECT ANY_VALUE(price)       AS price,
                             ANY_VALUE(id)          AS id,
                             ANY_VALUE(goods_id)    AS goods_id,
                             ANY_VALUE(create_time) AS create_time
                      FROM mkt_price_control
                      WHERE 1 = 1
                        AND price_id = 1
                      ORDER BY create_time DESC
                      LIMIT 9999999
                  ) mpc1
             GROUP BY mpc1.goods_id
...
#            AND bc.id = ''
         ...
         FROM (
                  ...
                  FROM (
                           SELECT ANY_VALUE(tsrbl.id)             AS tsrblId,
                           ...
                           LEFT JOIN fin_cst_pac_item_costs fcpic
                                     ON (fcpic.sku_id = tt.skuId AND fcpic.base_company_id = tt.baseCompanyId
                                         AND fcpic.period_start_date <= tt.completeTime AND
                                         fcpic.period_end_date >= tt.completeTime)
                  WHERE 2 = '1'
                  GROUP BY tt.tsrblId
              ) tab
         GROUP BY tab.field_id
         UNION ALL
         SELECT 3                                     AS seq,
                '货龄'                                  AS huoLing,
