...
FROM
	...
	FROM
		...
		LEFT JOIN bas_shop a ON t.shop_id = a.id AND (a.accounting_company_id IN ('${ID}') OR 1=1 )
	...
	WHERE
		t.stock_type = 2
		AND t.cons_status = 'Y'
		AND ((DATE_FORMAT(t.delivery_time,'%Y-%m') BETWEEN '${CREATE_TIME_START}' AND '${CREATE_TIME_END}'))
	GROUP BY
		a.CODE,
		a.NAME UNION ALL
	SELECT
