...
FROM (SELECT a.id, c.aid, c.cid, c.name AS NAME
     ...
     (SELECT a.summary_item_id                    AS goods_id,
      ...
      WHERE a.stock_type = 1 -- 账户类型
        AND d.id = 1
        AND (DATE_FORMAT(a.create_time, '%Y-%m') BETWEEN '${E3PLUS_QC_CREATE_TIME_START}' AND '${E3PLUS_QC_CREATE_TIME_END}')
     ...
     ON n.goods_id = a.id
WHERE 1 = 1
  AND IF('${E3PLUS_QC_ID}' = '', 1 = 1, cid IN ('${replace(E3PLUS_QC_ID,",","', '")}'))
GROUP BY a.`name`, cid
HAVING CASE
           WHEN '${E3PLUS_QC_BALANCE_AMOUNT}' IS NULL OR '${E3PLUS_QC_BALANCE_AMOUNT}' = 0 OR '${E3PLUS_QC_BALANCE_AMOUNT}' = 'null'
               THEN 1 = 1
           ELSE BALANCE_AMOUNT != 0
    END
   AND CASE
           WHEN '${E3PLUS_QC_BALANCE_PRICE}' IS NULL OR '${E3PLUS_QC_BALANCE_PRICE}' = 0 OR '${E3PLUS_QC_BALANCE_PRICE}' = 'null'
               THEN 1 = 1
