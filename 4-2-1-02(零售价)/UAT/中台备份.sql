...
( SELECT    a.id,  c.id cid, c.name AS NAME  FROM   gds_goods a 
    ...
    LEFT JOIN
        ...
        WHERE c.CODE = '00000001' 
             ...
             AND f.id = 1 
			 AND (((DATE_FORMAT(a.create_time,'%Y-%m-%d')) <= '@CREATE_TIME_END') OR ('@CREATE_TIME_END'='@'))
			 AND (((DATE_FORMAT(a.create_time,'%Y-%m-%d')) >= '@CREATE_TIME_START') OR ('@CREATE_TIME_START'='@'))
         GROUP BY a.`goods_id`) n
        ON n.goods_id = a.id
WHERE 1 = 1 AND  (cid IN ('@ID') OR '@ID'='@')
GROUP BY a.`name`,cid 
HAVING
    CASE
        WHEN ''@BALANCE_AMOUNT''  IS NULL OR ''@BALANCE_AMOUNT'' = '[0]' OR  ''@BALANCE_PRICE'' = 'null'
        THEN 1 = 1
