 SELECT
   ...
   a.cid
FROM
( SELECT    a.id,  c.id cid, c.name AS NAME  FROM   gds_goods a
    INNER JOIN gds_goods_base_attribute b ON a.id = b.goods_id
    INNER  JOIN (Select a.id,b.name From gds_category_tree a, (select id,name From gds_category_tree where level_num=1) b where a.parent_id=b.id) c
    ON b.`category_tree_id` = c.`id`) a
    LEFT JOIN
        ( SELECT
            a.summary_item_id as goods_id,
            SUM(a.begin_qty) AS d,
            ROUND(SUM(a.begin_qty * c.price), 2) AS d1
         FROM
            fin_cst_pac_item_costs_summary a -- PAC商品成本表
        ...
        WHERE a.stock_type = 1 -- 账户类型
             AND d.id = 1
             AND ((DATE_FORMAT(a.create_time,'%Y-%m') BETWEEN '2022-01' AND '2022-04') OR ('2022-01'='@' AND '2022-04'='@'))
    ...
    LEFT JOIN
        ...
        WHERE c.CODE = '00000001'
             ...
             AND f.id = 1
			 AND (((DATE_FORMAT(a.create_time,'%Y-%m-%d')) <= '2022-04') OR ('2022-04'='@'))
			 AND (((DATE_FORMAT(a.create_time,'%Y-%m-%d')) >= '2022-01') OR ('2022-01'='@'))
         GROUP BY a.`goods_id`) n
        ON n.goods_id = a.id
WHERE 1 = 1 AND  (cid IN (0) OR 1=1)
GROUP BY a.`name`,cid
HAVING
    CASE
        WHEN 'null'  IS NULL OR 'null' = '[0]' OR  'null' = 'null'
        THEN 1 = 1
